...
nodeVariableDeclarations::nodeVariableDeclarations(Node *vd):
    Node("<vardecs> --> <vardec>", 1)
{
    vardec = vd;
}
nodeVariableDeclarations::nodeVariableDeclarations(Node *vds, Node *vd):
    Node("<vardecs> --> <vardecs> <vardec>", 2)
{
    vardecs = vds;
    vardec = vd;
}
void nodeVariableDeclarations::print(ostream *out, string place)
{
    switch(prodType) {
        case 1:
            *out << place + label << endl;
...
nodeVarDec::nodeVarDec(Node *t, Node *id, int pt):
...
{}
nodeSimpleType::nodeSimpleType(Node *id) :
...
nodeMethDec::nodeMethDec(Node *rt, Node *id, Node *plist, Node *b) :
    Node("<methdec> --> <resulttype> ID ( <paramlist> ) <block>", 1)
{
    resultType = rt;
    ident = id;
    parameterList = plist;
...
nodeParameter::nodeParameter(Node *t, Node *id) :
...
nodeName::nodeName(int pt) :
    Node("<name> --> THIS", pt)
{}
nodeName::nodeName(Node* id, int pt) :
    Node("<name> --> ID", pt)
{
    ident = id;
}
    ...
    Node("<exp> --> ? ?", pt)
{
    switch(prodType) {
        case 4:
            ...
            break;
        case 7:
            setLabel("<exp> --> <unaryop> <exp>");
            unaryop = fnode;
            fexp = fnode;
            break;
    }
}
nodeExpression::nodeExpression(Node *fnode, Node *snode, Node *tnode, int pt) :
    Node("<exp> --> ? ? ?", pt)
{
    switch(prodType) {
        case 8:
            setLabel("<exp> --> <exp> <relationop> <exp>");
            fexp = fnode;
            relationop = snode;
            sexp = tnode;
            break;
        case 9:
            setLabel("<exp> --> <exp> <sumop> <exp>");
            fexp = fnode;
            sumop = snode;
            sexp = tnode;
            break;
        case 10:
            setLabel("<exp> --> <exp> <productop> <exp>");
            fexp = fnode;
            productop = snode;
            sexp = tnode;
            break;
    ...
    *out << place + label << endl;
    switch(prodType) {
        ...
        case 6:
            ...
            break;
        case 7:
            unaryop->print(out, place + "  ");
            fexp->print(out, place + "  ");
            break;
        case 8:
            fexp->print(out, place + "  ");
            relationop->print(out, place + "  ");
            sexp->print(out, place + "  ");
            break;
        case 9:
            fexp->print(out, place + "  ");
            sumop->print(out, place + "  ");
            sexp->print(out, place + "  ");
            break;
        case 10:
            fexp->print(out, place + "  ");
            productop->print(out, place + "  ");
            sexp->print(out, place + "  ");
            break;
        case 11:
            fexp->print(out, place + "  ");
    ...
    Node("<newexp> --> ? ?", pt)
{
    switch(prodType){
        case 1:
            ...
            break;
        case 2:
            setLabel("<newexp> --> NEW <simpletype> [ <exp> ] [ ]");
            simpletype = fnode;
            expression = snode;
            break;
    }
